...
RUN wget -q https://github.com/microsoft/Docker-Provider/releases/download/1.14/azure-mdsd_1.14.2-build.master.284_x86_64.deb  &&  /usr/bin/dpkg -i azure-mdsd*.deb  &&  rm -f azure-mdsd*.deb
...
RUN  cp -f mdsd.xml /etc/mdsd.d  &&  cp -f envmdsd /etc/mdsd.d
