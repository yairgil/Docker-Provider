...
func Test_duplicate_addressable_map(t *testing.T) {
	...
	qs1_copy := qs1.duplicate()
	assert(t, qs1_copy.len() == 0, `qs1_copy.len() == 0`)
	...
	*third_index = 3
	...
	qs2_copy := qs2.duplicate()
	assert(t, qs2_copy.len() == 3, `qs2_copy.len() == 3`)

	// make sure the copy and original don't share any memory
	qs2.delete("gonna")
	assert(t, qs2_copy.len() == 3, `qs2_copy.len() == 3`)
}
