  ...
  class Kube_PodInventory_Input < Input
    ...
    def start
      if @run_interval
        ...
        $log.info("in_kube_podinventory::start : PODS_CHUNK_SIZE  @ #{@PODS_CHUNK_SIZE} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        $log.info("in_kube_podinventory::start : PODS_EMIT_STREAM_BATCH_SIZE  @ #{@PODS_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        $log.info("in_kube_podinventory::start : isDisableKubeContainerPerf=#{@isDisableKubeContainerPerf} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        $log.info("in_kube_podinventory::start : isDisableKubeGPUPerf=#{@isDisableKubeGPUPerf} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        $log.info("in_kube_podinventory::start : isDisableKubeWinContainerInventory=#{@isDisableKubeWinContainerInventory} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        $log.info("in_kube_podinventory::start : isDiableKubeServices=#{@isDiableKubeServices} @ #{Time.now.utc.round(10).iso8601(10)}")

        if !ENV["DISABLE_KPI_MDM"].nil? && !ENV["DISABLE_KPI_MDM"].empty? && ENV["DISABLE_KPI_MDM"].downcase == "true".downcase
          @isDisableMDM = true
        end
        $log.info("in_kube_podinventory::start : isDisableMDM=#{@isDisableMDM} @ #{Time.now.utc.round(10).iso8601(10)}")

        if !ENV["DISABLE_KPI_ALL"].nil? && !ENV["DISABLE_KPI_ALL"].empty? && ENV["DISABLE_KPI_ALL"].downcase == "true".downcase
          @isDisableKPIAll = true
        end
        $log.info("in_kube_podinventory::start : isDisableKPIAll=#{@isDisableKPIAll} @ #{Time.now.utc.round(10).iso8601(10)}")
    ...
    def enumerate(podList = nil)
      if @isDisableKPIAll
        $log.info("in_kube_podinventory::enumerate: ***isDisableKPIAll*** @ #{Time.now.utc.round(10).iso8601(10)}")
      else
        begin
          ...
          $log.info("in_kube_podinventory::enumerate : Getting services from Kube API @ #{Time.now.utc.round(10).iso8601(10)}")
          ...
          $log.info("in_kube_podinventory::enumerate : Done getting services from Kube API @ #{Time.now.utc.round(10).iso8601(10)}")

          if !serviceInfo.nil?
            $log.info("in_kube_podinventory::enumerate:Start:Parsing services data using yajl @ #{Time.now.utc.round(10).iso8601(10)}")
            serviceList = Yajl::Parser.parse(StringIO.new(serviceInfo.body))
            $log.info("in_kube_podinventory::enumerate:End:Parsing services data using yajl @ #{Time.now.utc.round(10).iso8601(10)}")
            serviceInfo = nil
            # service inventory records much smaller and fixed size compared to serviceList
            $log.info("in_kube_podinventory::enumerate:Start:getKubeServicesInventoryRecords @ #{Time.now.utc.round(10).iso8601(10)}")
            ...
            serviceList = nil
            $log.info("in_kube_podinventory::enumerate:End:getKubeServicesInventoryRecords @ #{Time.now.utc.round(10).iso8601(10)}")
          end
          ...
          $log.info("in_kube_podinventory::enumerate : Getting pods from Kube API @ #{Time.now.utc.round(10).iso8601(10)}")
          continuationToken, podInventory = KubernetesApiClient.getResourcesAndContinuationToken("pods?limit=#{@PODS_CHUNK_SIZE}")
          $log.info("in_kube_podinventory::enumerate : Done getting pods from Kube API @ #{Time.now.utc.round(10).iso8601(10)}")
          ...
          if (!podInventory.nil? && !podInventory.empty? && podInventory.key?("items") && !podInventory["items"].nil? && !podInventory["items"].empty?)
            $log.info("in_kube_podinventory::enumerate : number of pod items :#{podInventory["items"].length}  from Kube API @ #{Time.now.utc.round(10).iso8601(10)}")
            $log.info("in_kube_podinventory::enumerate:Start:parse_and_emit_records @ #{Time.now.utc.round(10).iso8601(10)}")
            parse_and_emit_records(podInventory, serviceRecords, continuationToken, batchTime)
            $log.info("in_kube_podinventory::enumerate:End:parse_and_emit_records @ #{Time.now.utc.round(10).iso8601(10)}")
          else
            $log.warn "in_kube_podinventory::enumerate:Received empty podInventory @ #{Time.now.utc.round(10).iso8601(10)}"
          end
          ...
          if (timeDifferenceInMinutes >= 5)
            telemetryFlush = true
            $log.info("in_kube_podinventory::enumerate:set telemetryFlush to true @ #{Time.now.utc.round(10).iso8601(10)}")
          end

          # Flush AppInsights telemetry once all the processing is done
          if telemetryFlush == true
            $log.info("in_kube_podinventory::enumerate:Start:telemetryFlush @ #{Time.now.utc.round(10).iso8601(10)}")
            telemetryProperties = {}
            ...
            @@podTelemetryTimeTracker = DateTime.now.to_time.to_i
            $log.info("in_kube_podinventory::enumerate:End:telemetryFlush @ #{Time.now.utc.round(10).iso8601(10)}")
          end
        rescue => errorStr
          $log.warn "in_kube_podinventory::enumerate:Failed in enumerate: #{errorStr} @ #{Time.now.utc.round(10).iso8601(10)}"
    ...
    def parse_and_emit_records(podInventory, serviceRecords, continuationToken, batchTime = Time.utc.iso8601)
      ...
      begin #begin block start
        ...
        if !@isDisableKubeWinContainerInventory
          $log.info "in_kube_podinventory::parse_and_emit_records:Start:getWindowsNodesArray@ #{Time.now.utc.round(10).iso8601(10)}"
          winNodes = KubernetesApiClient.getWindowsNodesArray
          $log.info "in_kube_podinventory::parse_and_emit_records:End:getWindowsNodesArray @ #{Time.now.utc.round(10).iso8601(10)}"
        end

        podInventory["items"].each do |item| #podInventory block start
          # pod inventory records
          podName = item["metadata"]["name"]
          $log.info "in_kube_podinventory::parse_and_emit_records:Start:getPodInventoryRecords:podName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
          podInventoryRecords = getPodInventoryRecords(item, serviceRecords, batchTime)
          $log.info "in_kube_podinventory::parse_and_emit_records:End:getPodInventoryRecords:podName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"

          podInventoryRecords.each do |record|
            $log.info "in_kube_podinventory::parse_and_emit_records:Start:podInventoryRecords.each:PodName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
            if !record.nil?
              wrapper = {
              ...
              if !@isDisableMDM
                $log.info "in_kube_podinventory::parse_and_emit_records:Start:process_pod_inventory_record - PodName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
                @inventoryToMdmConvertor.process_pod_inventory_record(wrapper)
                $log.info "in_kube_podinventory::parse_and_emit_records:End:process_pod_inventory_record - PodName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
              end
            end
            $log.info "in_kube_podinventory::parse_and_emit_records:End:podInventoryRecords.each:PodName: #{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
          end
          ...
          if !@isDisableKubeWinContainerInventory
            if winNodes.length > 0
              $log.info "in_kube_podinventory::parse_and_emit_records:Start: get windows container inventory records since winNodes length: #{winNodes.length } @ #{Time.now.utc.round(10).iso8601(10)}"
              nodeName = ""
              ...
              if (!nodeName.empty? && (winNodes.include? nodeName))
                ...
                $log.info "in_kube_podinventory::parse_and_emit_records:getContainerInventoryRecords:start - nodeName: #{nodeName} and podName:#{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
                ...
                $log.info "in_kube_podinventory::parse_and_emit_records:getContainerInventoryRecords:end - nodeName: #{nodeName} and podName:#{podName} @ #{Time.now.utc.round(10).iso8601(10)}"
              ...
              $log.info "in_kube_podinventory::parse_and_emit_records:End: get windows container inventory records since winNodes length: #{winNodes.length } @ #{Time.now.utc.round(10).iso8601(10)}"
            end
          end

          if @PODS_EMIT_STREAM_BATCH_SIZE > 0 && eventStream.count >= @PODS_EMIT_STREAM_BATCH_SIZE
            $log.info("in_kube_podinventory::parse_and_emit_records:Start:number of pod and windows container inventory records emitted #{eventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            if (!@@istestvar.nil? && !@@istestvar.empty? && @@istestvar.casecmp("true") == 0)
              $log.info("kubePodInventoryEmitStreamSuccess @ #{Time.now.utc.round(10).iso8601(10)}")
            end
            router.emit_stream(@tag, eventStream) if eventStream
            $log.info("in_kube_podinventory::parse_and_emit_records:End:number of pod and windows container inventory records emitted #{eventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            eventStream = MultiEventStream.new
          end
          ...
          if !@isDisableKubeContainerPerf
            $log.info("in_kube_podinventory::parse_and_emit_records:Start:getContainerResourceRequestsAndLimits @ #{Time.now.utc.round(10).iso8601(10)}")
            ...
            $log.info("in_kube_podinventory::parse_and_emit_records:End:getContainerResourceRequestsAndLimits- containerMetricDataItems: #{containerMetricDataItems.length} @ #{Time.now.utc.round(10).iso8601(10)}")
          end
          $log.info("in_kube_podinventory::parse_and_emit_records:Start:Adding metric data items to the kubePerfEventStream : #{containerMetricDataItems.length} @ #{Time.now.utc.round(10).iso8601(10)}")
          ...
          $log.info("in_kube_podinventory::parse_and_emit_records:End:Adding metric data items to the kubePerfEventStream : #{containerMetricDataItems.length} @ #{Time.now.utc.round(10).iso8601(10)}")

          if @PODS_EMIT_STREAM_BATCH_SIZE > 0 && kubePerfEventStream.count >= @PODS_EMIT_STREAM_BATCH_SIZE
            $log.info("in_kube_podinventory::parse_and_emit_records:Start: number of container perf records emitted #{kubePerfEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            router.emit_stream(@@kubeperfTag, kubePerfEventStream) if kubePerfEventStream
            if (!@@istestvar.nil? && !@@istestvar.empty? && @@istestvar.casecmp("true") == 0)
              $log.info("kubeContainerPerfEventEmitStreamSuccess @ #{Time.now.utc.round(10).iso8601(10)}")
            end
            $log.info("in_kube_podinventory::parse_and_emit_records:End:number of container perf records emitted #{kubePerfEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            kubePerfEventStream = MultiEventStream.new
          end
          ...
          if !@isDisableKubeGPUPerf
            $log.info("in_kube_podinventory::parse_and_emit_records:Start:getContainerResourceRequestsAndLimitsAsInsightsMetrics @ #{Time.now.utc.round(10).iso8601(10)}")
            ...
            $log.info("in_kube_podinventory::parse_and_emit_records:End:getContainerResourceRequestsAndLimitsAsInsightsMetrics @ #{Time.now.utc.round(10).iso8601(10)}")
          end

          $log.info("in_kube_podinventory::parse_and_emit_records:Start:Adding GPU metric data items to the insightsMetricsEventStream : #{containerGPUInsightsMetricsDataItems.length} @ #{Time.now.utc.round(10).iso8601(10)}")
          ...
          if @PODS_EMIT_STREAM_BATCH_SIZE > 0 && insightsMetricsEventStream.count >= @PODS_EMIT_STREAM_BATCH_SIZE
            $log.info("in_kube_podinventory::parse_and_emit_records:Start:number of GPU insights metrics records emitted #{insightsMetricsEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            if (!@@istestvar.nil? && !@@istestvar.empty? && @@istestvar.casecmp("true") == 0)
              $log.info("kubePodInsightsMetricsEmitStreamSuccess @ #{Time.now.utc.round(10).iso8601(10)}")
            end
            router.emit_stream(Constants::INSIGHTSMETRICS_FLUENT_TAG, insightsMetricsEventStream) if insightsMetricsEventStream
            $log.info("in_kube_podinventory::parse_and_emit_records:End:number of GPU insights metrics records emitted #{insightsMetricsEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
            insightsMetricsEventStream = MultiEventStream.new
          end
        ...
        if kubePerfEventStream.count > 0
          $log.info("in_kube_podinventory::parse_and_emit_records:Start:number of perf records emitted #{kubePerfEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
          router.emit_stream(@@kubeperfTag, kubePerfEventStream) if kubePerfEventStream
          $log.info("in_kube_podinventory::parse_and_emit_records:End:number of perf records emitted #{kubePerfEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        if insightsMetricsEventStream.count > 0
          $log.info("in_kube_podinventory::parse_and_emit_records:Start:number of insights metrics records emitted #{insightsMetricsEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
          router.emit_stream(Constants::INSIGHTSMETRICS_FLUENT_TAG, insightsMetricsEventStream) if insightsMetricsEventStream
          if (!@@istestvar.nil? && !@@istestvar.empty? && @@istestvar.casecmp("true") == 0)
            $log.info("kubePodInsightsMetricsEmitStreamSuccess @ #{Time.now.utc.round(10).iso8601(10)}")
          end
          $log.info("in_kube_podinventory::parse_and_emit_records:End:number of insights metrics records emitted #{insightsMetricsEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
        ...
        if continuationToken.nil? #no more chunks in this batch to be sent, get all mdm pod inventory records to send
          if !@isDisableMDM
            @log.info "START: Sending pod inventory mdm records to out_mdm @ #{Time.now.utc.round(10).iso8601(10)}"
            ...
            @log.info "END: Sending pod inventory mdm records to out_mdm @ #{Time.now.utc.round(10).iso8601(10)}"
         end
        end

        if continuationToken.nil? # sending kube services inventory records
          if !@isDiableKubeServices
            @log.info "START: Sending kube services inventory records @ #{Time.now.utc.round(10).iso8601(10)}"
            ...
            serviceRecords.each do |kubeServiceRecord|
              if !kubeServiceRecord.nil?
                ...
                if @PODS_EMIT_STREAM_BATCH_SIZE > 0 && kubeServicesEventStream.count >= @PODS_EMIT_STREAM_BATCH_SIZE
                  $log.info("in_kube_podinventory::parse_and_emit_records: number of service records emitted #{kubeServicesEventStream.count} @ #{Time.now.utc.round(10).iso8601(10)}")
                  router.emit_stream(@@kubeservicesTag, kubeServicesEventStream) if kubeServicesEventStream
                  kubeServicesEventStream = MultiEventStream.new
                  if (!@@istestvar.nil? && !@@istestvar.empty? && @@istestvar.casecmp("true") == 0)
                    $log.info("kubeServicesEventEmitStreamSuccess @ #{Time.now.utc.round(10).iso8601(10)}")
                  end
                end
              end
            end
          ...
          @log.info "END: Sending kube services inventory records @ #{Time.now.utc.round(10).iso8601(10)}"
        end
    ...
    def run_periodic
      ...
      until done
        ...
        if !done
          begin
            $log.info("in_kube_podinventory::run_periodic.enumerate.start #{Time.now.utc.round(10).iso8601(10)}")
            enumerate
            $log.info("in_kube_podinventory::run_periodic.enumerate.end #{Time.now.utc.round(10).iso8601(10)}")
          rescue => errorStr
    ...
    def getPodInventoryRecords(item, serviceRecords, batchTime = Time.utc.iso8601)
      records = []
      record = {}
      $log.info "in_kube_podinventory::getPodInventoryRecords:start @ #{Time.now.utc.round(10).iso8601(10)}"
      begin
        record["CollectionTime"] = batchTime #This is the time that is mapped to become TimeGenerated
        record["Name"] = item["metadata"]["name"]
        podNameSpace = item["metadata"]["namespace"]
        $log.info "in_kube_podinventory::getPodInventoryRecords:getPodUid - start @ #{Time.now.utc.round(10).iso8601(10)}"
        podUid = KubernetesApiClient.getPodUid(podNameSpace, item["metadata"])
        $log.info "in_kube_podinventory::getPodInventoryRecords:getPodUid - end @ #{Time.now.utc.round(10).iso8601(10)}"
        if podUid.nil?
          return records
        end
        ...
        $log.info "in_kube_podinventory::getPodInventoryRecords:isAROv3MasterOrInfraPod - start @ #{Time.now.utc.round(10).iso8601(10)}"
        if KubernetesApiClient.isAROv3MasterOrInfraPod(nodeName)
          return records
        end
        $log.info "in_kube_podinventory::getPodInventoryRecords:isAROv3MasterOrInfraPod - end @ #{Time.now.utc.round(10).iso8601(10)}"
        ...
        $log.info "in_kube_podinventory::getPodInventoryRecords:get clusterId and name from KubernetesApiClient - start @ #{Time.now.utc.round(10).iso8601(10)}"

        record["Computer"] = nodeName
        record["ClusterId"] = KubernetesApiClient.getClusterId
        record["ClusterName"] = KubernetesApiClient.getClusterName

        $log.info "in_kube_podinventory::getPodInventoryRecords:get clusterId and name from KubernetesApiClient - end @ #{Time.now.utc.round(10).iso8601(10)}"
        ...
        if !@isDisableMDM
          $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_pods_ready_metric - start @ #{Time.now.utc.round(10).iso8601(10)}"
          ...
          $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_pods_ready_metric - end @ #{Time.now.utc.round(10).iso8601(10)}"
        end
          ...
          podContainers.each do |container|
            ...
            if containerStatus.keys[0] == "running"
              ...
              if !record["ControllerKind"].nil? && record["ControllerKind"].downcase == Constants::CONTROLLER_KIND_JOB
                if !@isDisableMDM
                  $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_terminated_job_metric - start @ #{Time.now.utc.round(10).iso8601(10)}"
                  @inventoryToMdmConvertor.process_record_for_terminated_job_metric(record["ControllerName"], record["Namespace"], containerStatus)
                  $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_terminated_job_metric - end @ #{Time.now.utc.round(10).iso8601(10)}"
                end
              end
            end
            ...
            begin
              if !container["lastState"].nil? && container["lastState"].keys.length == 1
                ...
                if lastStateObject.key?("reason") && lastStateObject.key?("startedAt") && lastStateObject.key?("finishedAt")
                  ...
                  if lastStateReason.downcase == Constants::REASON_OOM_KILLED
                    if !@isDisableMDM
                      $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_oom_killed_metric - start @ #{Time.now.utc.round(10).iso8601(10)}"
                      @inventoryToMdmConvertor.process_record_for_oom_killed_metric(record["ControllerName"], record["Namespace"], lastFinishedTime)
                      $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_oom_killed_metric - end @ #{Time.now.utc.round(10).iso8601(10)}"
                    end
                  end
                  lastStateReason = nil
                else
              ...
              if (!containerRestartCount.nil? && (containerRestartCount.is_a? Integer) && containerRestartCount > 0)
                if !@isDisableMDM
                  $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_container_restarts_metric - start @ #{Time.now.utc.round(10).iso8601(10)}"
                  ...
                  $log.info "in_kube_podinventory::getPodInventoryRecords:process_record_for_container_restarts_metric - end @ #{Time.now.utc.round(10).iso8601(10)}"
                end
              end
            rescue => errorStr
      ...
      $log.info "in_kube_podinventory::getPodInventoryRecords:end @ #{Time.now.utc.round(10).iso8601(10)}"
      return records
    end
