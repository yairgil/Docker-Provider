...
module Fluent
  class Kube_nodeInventory_Input < Input
    ...
    def start
      if @run_interval
        if !ENV["NODES_ENABLE_V2"].nil? && !ENV["NODES_ENABLE_V2"].empty?
          @NODES_ENABLE_V2 = ENV["NODES_ENABLE_V2"].to_s.downcase == "true" ? true : false
        end
        $log.info("in_kube_nodes::start : NODES_ENABLE_V2  @ #{@NODES_ENABLE_V2}")
      ...
      begin
        ...
        if (!nodeInventory.nil? && !nodeInventory.empty? && nodeInventory.key?("items") && !nodeInventory["items"].nil? && !nodeInventory["items"].empty?)
          if @ENABLE_PARSE_AND_EMIT
            if @NODES_ENABLE_V2
              parse_and_emit_records_v2(nodeInventory, batchTime)
            else
    ...
    def get_node_telemetry_props(item)
      properties = {}
    ...
    def parse_and_emit_records_v2(nodeInventory, batchTime = Time.utc.iso8601)
      begin
        $log.info "in_kube_nodes::parse_and_emit_records_v2:start #{Time.now.utc.iso8601}"
        currentTime = Time.now
        ...
        nodeInventory["items"].each do |item|
          # node inventory
          nodeInventoryRecord = get_node_inventory_record(item, batchTime)
          wrapper = {
          ...
          if @NODES_EMIT_STREAM_BATCH_SIZE > 0 && eventStream.count >= @NODES_EMIT_STREAM_BATCH_SIZE
            if @NODES_EMIT_STREAM
              $log.info("in_kube_node::parse_and_emit_records_v2: number of node inventory records emitted #{@NODES_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.iso8601}")
              router.emit_stream(@tag, eventStream) if eventStream
            end
            if @MDM_KUBE_NODE_INVENTORY_EMIT_STREAM
              $log.info("in_kube_node::parse_and_emit_records_v2: number of mdm node inventory records emitted #{@NODES_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.iso8601}")
          ...
          containerNodeInventoryRecord = get_container_node_inventory_record(item, batchTime)
          ...
          if @NODES_EMIT_STREAM_BATCH_SIZE > 0 && containerNodeInventoryEventStream.count >= @NODES_EMIT_STREAM_BATCH_SIZE
            if @CONTAINER_NODE_INVENTORY_EMIT_STREAM
              $log.info("in_kube_node::parse_and_emit_records_v2: number of container node inventory records emitted #{@NODES_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.iso8601}")
          ...
          nodeMetricRecords = []
          nodeMetricRecord = KubernetesApiClient.parseNodeLimits_v2(item, "allocatable", "cpu", "cpuAllocatableNanoCores", batchTime)
          if !nodeMetricRecord.nil? && !nodeMetricRecord.empty?
            nodeMetricRecords.push(nodeMetricRecord)
          end
          nodeMetricRecord = KubernetesApiClient.parseNodeLimits_v2(item, "allocatable", "memory", "memoryAllocatableBytes", batchTime)
          if !nodeMetricRecord.nil? && !nodeMetricRecord.empty?
            nodeMetricRecords.push(nodeMetricRecord)
          end
          nodeMetricRecord = KubernetesApiClient.parseNodeLimits_v2(item, "capacity", "cpu", "cpuCapacityNanoCores", batchTime)
          if !nodeMetricRecord.nil? && !nodeMetricRecord.empty?
            nodeMetricRecords.push(nodeMetricRecord)
          end
          nodeMetricRecord = KubernetesApiClient.parseNodeLimits_v2(item, "capacity", "memory", "memoryCapacityBytes", batchTime)
          if !nodeMetricRecord.nil? && !nodeMetricRecord.empty?
            nodeMetricRecords.push(nodeMetricRecord)
          end
          ...
          if @NODES_EMIT_STREAM_BATCH_SIZE > 0 && kubePerfEventStream.count >= @NODES_EMIT_STREAM_BATCH_SIZE
            if @NODES_PERF_EMIT_STREAM
              $log.info("in_kube_nodes::parse_and_emit_records_v2: number of node perf metric records emitted #{@NODES_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.iso8601}")
          ...
          insightsMetricsRecord = KubernetesApiClient.parseNodeLimitsAsInsightsMetrics_v2(item, "allocatable", "nvidia.com/gpu", "nodeGpuAllocatable", batchTime)
          if !insightsMetricsRecord.nil? && !insightsMetricsRecord.empty?
            nodeGPUInsightsMetricsRecords.push(insightsMetricsRecord)
          end
          insightsMetricsRecord = KubernetesApiClient.parseNodeLimitsAsInsightsMetrics_v2(item, "capacity", "nvidia.com/gpu", "nodeGpuCapacity", batchTime)
          if !insightsMetricsRecord.nil? && !insightsMetricsRecord.empty?
            nodeGPUInsightsMetricsRecords.push(insightsMetricsRecord)
          end
          insightsMetricsRecord = KubernetesApiClient.parseNodeLimitsAsInsightsMetrics_v2(item, "allocatable", "amd.com/gpu", "nodeGpuAllocatable", batchTime)
          if !insightsMetricsRecord.nil? && !insightsMetricsRecord.empty?
            nodeGPUInsightsMetricsRecords.push(insightsMetricsRecord)
          end
          insightsMetricsRecord = KubernetesApiClient.parseNodeLimitsAsInsightsMetrics_v2(item, "capacity", "amd.com/gpu", "nodeGpuCapacity", batchTime)
          if !insightsMetricsRecord.nil? && !insightsMetricsRecord.empty?
            nodeGPUInsightsMetricsRecords.push(insightsMetricsRecord)
          end
          ...
          if @NODES_EMIT_STREAM_BATCH_SIZE > 0 && insightsMetricsEventStream.count >= @NODES_EMIT_STREAM_BATCH_SIZE
            if @GPU_NODES_PERF_EMIT_STREAM
              $log.info("in_kube_nodes::parse_and_emit_records_v2: number of GPU node perf metric records emitted #{@NODES_EMIT_STREAM_BATCH_SIZE} @ #{Time.now.utc.iso8601}")
          ...
          if (timeDifferenceInMinutes >= Constants::TELEMETRY_FLUSH_INTERVAL_IN_MINUTES)
            properties = get_node_telemetry_props(item)
        ...
        if eventStream.count > 0
          if @NODES_PERF_EMIT_STREAM
            $log.info("in_kube_node::parse_and_emit_records_v2: number of node inventory records emitted #{eventStream.count} @ #{Time.now.utc.iso8601}")
            router.emit_stream(@tag, eventStream) if eventStream
          end
          eventStream = nil
        end
        if kubePerfEventStream.count > 0
          if @NODES_PERF_EMIT_STREAM
            $log.info("in_kube_nodes::parse_and_emit_records_v2: number of node perf metric records emitted #{kubePerfEventStream.count} @ #{Time.now.utc.iso8601}")
            router.emit_stream(@@kubeperfTag, kubePerfEventStream) if kubePerfEventStream
          end
        ...
        if insightsMetricsEventStream.count > 0
          if @GPU_NODES_PERF_EMIT_STREAM
            $log.info("in_kube_nodes::parse_and_emit_records_v2: number of GPU node perf metric records emitted #{insightsMetricsEventStream.count} @ #{Time.now.utc.iso8601}")
            router.emit_stream(Constants::INSIGHTSMETRICS_FLUENT_TAG, insightsMetricsEventStream) if insightsMetricsEventStream
          end
      ...
      $log.info "in_kube_nodes::parse_and_emit_records_v2:End #{Time.now.utc.iso8601}"
    end
        ...
        begin
          if @NODES_PERF_EMIT_STREAM
            if @NODES_PERF_EMIT_STREAM_SPLIT_ENABLE
              ...
              # node cpu allocatable perf records
              kubePerfEventStream = MultiEventStream.new
              nodeMetricDataItems = []
              ...
              # node memory allocatable perf records
              kubePerfEventStream = MultiEventStream.new
              ...
              # node cpu capacity perf records
              kubePerfEventStream = MultiEventStream.new
              ...
              # node memory capacity perf records
              kubePerfEventStream = MultiEventStream.new
