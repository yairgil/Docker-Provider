      ...
      begin
        if @process_incoming_stream
          ...
          if object_name == Constants::OBJECT_NAME_K8S_NODE && @metrics_to_collect_hash.key?(counter_name.downcase)
            ...
            @log.info "allocatable_percentage_metric_value for metric: #{metric_name} for instance: #{record["Host"]} percentage: #{allocatable_percentage_metric_value}"
      ...
      if @@controller_type.downcase == "replicaset"
        ...
        if !nodeInventory.nil?
          ...
          if !cpu_allocatable_json.nil?
            @cpu_allocatable = JSON.parse(cpu_allocatable_json[0]["json_Collections"])[0]["Value"]
          ...
          if !memory_allocatable_json.nil?
            @memory_allocatable = JSON.parse(memory_allocatable_json[0]["json_Collections"])[0]["Value"]
