        ...
        def parse_file
            ...
            temp_model.each { |entry|
                ...
                if parent_monitor_id.is_a?(Array)
                    ...
                    @health_model_definition[monitor_id] = {"conditions" => conditions, "labels" => labels, "aggregation_algorithm" => aggregation_algorithm}
                elsif parent_monitor_id.is_a?(String)
                    @health_model_definition[monitor_id] = {"parent_monitor_id" => parent_monitor_id, "labels" => labels, "aggregation_algorithm" => aggregation_algorithm}
                elsif parent_monitor_id.nil?
                    @health_model_definition[monitor_id] = {"parent_monitor_id" => nil, "labels" => labels, "aggregation_algorithm" => aggregation_algorithm}
                end
            }
            @health_model_definition
        end
