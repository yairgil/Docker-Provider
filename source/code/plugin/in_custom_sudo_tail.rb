...
module Fluent
  class CustomSudoTail < Input
    Plugin.register_input('custom_sudo_tail', self)
    ...
    def set_system_command
      ...
      cName = "Unkown"
      tempContainerInfo = {}
      path = "/var/opt/microsoft/docker-cimprov/state/ContainerInventory/*"
      paths = ""
      Dir.glob(path).select { |p|
	cName = p.split('/').last;
	p = "/var/lib/docker/containers/" + cName + "/" + cName + "-json.log"
    ...
    def run_periodic
	  ...
	  set_system_command
          ...
          $log.error "custom_sudo_tail failed to run or shutdown child proces", error => $!.to_s, :error_class => $!.class.to_s
