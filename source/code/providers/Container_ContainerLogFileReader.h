...
private:
    ...
    static void ParseAndExec(string containerId, string paths, bool isstderr = false)
    {
		...
		{
			syslog(LOG_ERR, "Container_ContainerLogFileReader - ParseAndExec %s", e.what());
		}
		catch (...)
		{
			syslog(LOG_ERR, "Container_ContainerLogFileReader - ParseAndExec- Unknown exception");
		}
    }
...
public:
    ...
    static void LinkFilesToStream(string containerId)
    {
		...
		{
			syslog(LOG_ERR, "Container_ContainerLogFileReader - LinkFilesToStream %s", e.what());
		}
		catch (...)
		{
			syslog(LOG_ERR, "Container_ContainerLogFileReader - LinkFilesToStream- Unknown exception");
		}
    }
};
