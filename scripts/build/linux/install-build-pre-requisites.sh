...
install_go_lang()
{
  ...
  if [[ $goVersion == *go1.18.1* ]] ; then
    echo "found existing installation of go version 1.18.1 so skipping the installation of go"
  else
    echo "installing go 1.18.1 version ..."
    sudo curl -O https://dl.google.com/go/go1.18.1.linux-amd64.tar.gz
    sudo tar -xvf go1.18.1.linux-amd64.tar.gz
    sudo mv -f go /usr/local
    echo "set file permission for go bin"
    sudo chmod 744 /usr/local/go/bin
    echo "installation of go 1.18.1 completed."
    echo "installation of go 1.18.1 completed."
  fi
...
echo "installing build pre-requisites python, go 1.18.1, dotnet, powershell, build dependencies and docker completed"
