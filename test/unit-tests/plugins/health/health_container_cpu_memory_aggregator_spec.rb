...
describe "HealthContainerCpuMemoryAggregator spec" do
  it "dedupes and drops older records" do
    ...
    nodes = Oj.load(File.read(File.join(File.expand_path(File.dirname(__FILE__)), "nodes.json")))
    pods = Oj.load(File.read(File.join(File.expand_path(File.dirname(__FILE__)), "pods.json")))
    deployments = Oj.load(File.read(File.join(File.expand_path(File.dirname(__FILE__)), "deployments.json")))
