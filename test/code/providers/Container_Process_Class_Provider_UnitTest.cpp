...
protected:
    void Testk8EnumerateInstances()
    {
        ...
        for (unsigned i = 0; i < context.Size(); ++i)
        {
            ...
            CPPUNIT_ASSERT_EQUAL(wstring(hostname), context[i].GetProperty(L"Computer", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            ...
            CPPUNIT_ASSERT_EQUAL(wstring(L"cptnamespace"), context[i].GetProperty(L"Namespace", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            ...
            CPPUNIT_ASSERT_EQUAL(wstring(L"k8_cpt.sandboxname_cptpodname_cptnamepsace_cptid"), context[i].GetProperty(L"Name", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
        }
        TestHelper::RunCommand("docker rm -f $(docker ps --filter \"name=k8_cpt.sandboxname_cptpodname_cptnamepsace_cptid\" -a -q)");
    }
    ...
    /*void TestNonk8EnumerateInstances()
    {
        ...
        for (unsigned i = 0; i < context.Size(); ++i)
        {
            ...
            CPPUNIT_ASSERT(instanceId.length());
            CPPUNIT_ASSERT_EQUAL(wstring(hostname), context[i].GetProperty(L"Computer", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            CPPUNIT_ASSERT_EQUAL(wstring(L"None"), context[i].GetProperty(L"Cmd", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            CPPUNIT_ASSERT_EQUAL(wstring(L"None"), context[i].GetProperty(L"Cmd", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            CPPUNIT_ASSERT_EQUAL(wstring(L"root"),context[i].GetProperty(L"Uid", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)));
            CPPUNIT_ASSERT(context[i].GetProperty(L"PID", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)).length());
            CPPUNIT_ASSERT(context[i].GetProperty(L"PPID", CALL_LOCATION(errMsg)).GetValue_MIString(CALL_LOCATION(errMsg)).length());
