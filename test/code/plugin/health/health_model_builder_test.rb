...
class FilterHealthModelBuilderTest < Test::Unit::TestCase
    ...
    #         mock_data_path = "C:/AzureMonitor/ContainerInsights/Docker-Provider/health_records/#{scenario}_daemon_set_signals.json"
    ...
    #         deployment_inventory = JSON.parse(File.read("C:/AzureMonitor/ContainerInsights/Docker-Provider/inventory/deployments.json"))
    ...
    #         provider = HealthMonitorProvider.new(cluster_id, cluster_labels, resources, "C:/AzureMonitor/ContainerInsights/Docker-Provider/installer/conf/healthmonitorconfig.json")
    ...
    #         serializer = HealthStateSerializer.new('C:\AzureMonitor\ContainerInsights\Docker-Provider\health_records\health_model_state.json')
    #         serializer.serialize(state)

    #         deserializer = HealthStateDeserializer.new('C:\AzureMonitor\ContainerInsights\Docker-Provider\health_records\health_model_state.json')
    #         deserialized_state = deserializer.deserialize

    #         after_state = HealthMonitorState.new
    #         # deserialized_state.each{|k,v|
    #         #     attrs = v.gsub(/(\n|#<struct HealthModel::HealthMonitorInstanceState\s+)/, '').gsub('<','')[0..-2]
    ...
    #         #     instance_state = HealthMonitorInstanceState.new(attrs)
    #         #}
    #         after_state.initialize_state(deserialized_state)

    #         puts "#{state.to_h == after_state.to_h}"
    #         puts scenario
    #     end
    ...
    def test_event_stream_aks_engine
        ...
        for scenario in 1..3
            mock_data_path = "C:/AzureMonitor/ContainerInsights/Docker-Provider/health_records/aks-engine/aks-engine-#{scenario}.json"
            file = File.read(mock_data_path)
            records = JSON.parse(file)
            ...
            deployment_inventory = JSON.parse(File.read("C:/AzureMonitor/ContainerInsights/Docker-Provider/inventory/aks-engine/deployments.json"))
            resources = HealthKubernetesResources.instance
            resources.node_inventory = node_inventory
            resources.pod_inventory = pod_inventory
            resources.deployment_inventory = deployment_inventory
