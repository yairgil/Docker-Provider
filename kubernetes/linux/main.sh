...
if [ "${CONTAINER_TYPE}" != "PrometheusSidecar" ]; then
      if [ ! -e "/etc/config/kube.conf" ]; then
         ...
         fluentd -vv -c /etc/fluent/container.conf -o /var/opt/microsoft/docker-cimprov/log/fluentd.log --log-rotate-age 5 --log-rotate-size 20971520 &
      else
        echo "*** starting fluentd v1 in replicaset"
        fluentd -vv -c /etc/fluent/kube.conf -o /var/opt/microsoft/docker-cimprov/log/fluentd.log --log-rotate-age 5 --log-rotate-size 20971520 &
      fi
fi
