...
#
# We expect this script to run from the BUILD directory (i.e. docker-osm/build).
...
if [ -z "$3" ]; then
	echo "Missing parameter: Container-package-name" >&2
	echo ""
	usage
	exit 1
fi
...
sed -e "s/CONTAINER_PKG=<CONTAINER_PKG>/CONTAINER_PKG=$3/" < primary.skel > primary.$$
mv primary.$$ primary.skel
